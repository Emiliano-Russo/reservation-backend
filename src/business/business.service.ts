...
export class BusinessService {
    ...
    async updateBusiness(id: string, businessUpdateDto: BusinessUpdateDto): Promise<any> {
        ...
        updateData.coordinates = businessUpdateDto.coordinates ?? business.coordinates;
        updateData.activePremiumSubscriptionId = businessUpdateDto.activePremiumSubscriptionId ?? business.activePremiumSubscriptionId;
        updateData.logoUrl = businessUpdateDto.logoUrl ?? business.logoURL;
        updateData.multimediaURL = businessUpdateDto.multimediaURL ?? business.multimediaURL;
        updateData.description = businessUpdateDto.description ?? business.description;
        updateData.assistantsID = businessUpdateDto.assistantsID ?? business.assistantsID;
        updateData.pendingInvitationsID = businessUpdateDto.pendingInvitationsID ?? business.pendingInvitationsID;
        updateData.status = businessUpdateDto.status ?? business.status;
        updateData.availability = businessUpdateDto.availability ?? business.availability;

        return Business.update(id, updateData);
    }

    async updateBusinessInvitation(id: string, businessUpdateDto: BusinessUpdateDto): Promise<any> {
        const business = await Business.get(id);
        ...
        var indexInvitationID = (updateData.pendingInvitationsID as Array<String>)?.indexOf(businessUpdateDto.selectedPendingInvitationID);

        if (businessUpdateDto.acceptInvitation) {
            if (indexInvitationID && indexInvitationID > -1) {
                (updateData.pendingInvitationsID as Array<string>).splice(indexInvitationID, indexInvitationID);
                (updateData.assistantsID as Array<string>).push(businessUpdateDto.selectedPendingInvitationID);
            }
        }
        else {
